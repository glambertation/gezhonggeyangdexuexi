...
4.1 SQuAD解决了吗
我们的阅读理解能解决所有SQuAD的例子或者 同级别的其他例子吗？
one
能预测一些问题，比如xxx的数量，名字。。
但是对例句的理解和区分答案上失败了。
...
用bert
模型会犯一些人类不会犯的错误。
可能很精细化的和text匹配，但是没有理解实体和事件之间，内在的结构。

three
如果段尾加一个分散的句子，我们的系统表现就大幅度降低（一半），这个分散句子重叠了问题，但没有和正确答案相冲突，也不会误导人理解。如果句子加入了不合文法的一串词，效果更差。
所以
1.现有模型 强依赖于 问题和段落的词汇线索
2.尽管原有模型达到了高精度，但他们对有挑战的例子还不够健壮、
这个是标准监督学习模型的一个重要问题，所以模型很难用于真实世界。

总结：尽管我们获得了高精度的结果，但现有模型知识专注于text的肤浅信息。其实高精度也说明 SQuAD的例子又简单又不需要理解

4.2 future work datasets
这两个数据库 CNN/DAILY MAIL and SQUAD
1.神经网络已经 可以可以达到这俩的 超人类天花板级别
2.这里的例子都比较简单不需要理解

SQUAD局限
1.问题是基于段落内容的
2.问题的回答 是单独的一个span，问题没有why和who ，都是yes no counting
3.单句答案 （可能会被指代系统解决掉）
...
4.3.1. 需求
1.速度和规模
替换lstm 用transformer 或者轻量级的 recurrent unit (SRU)
训练模型来填过文件中他们不需要读的部分
优化算法也影响收敛速度，多GPU和硬件也影响

2.健壮性
1。更对抗性的例子
2.transfer learning 和 多任务learning
3.打破监督学习的范式

3.可解释
以后的系统，不仅输出结构，还输出理由，这样读者可以决定他们是否需要输出结果、杠杆他们
但这种端到端的 是黑盒
这个在医药，法律领域特别管用

简单的办法就是 模型学会从输入中提取内容作为支持证据
复杂的就是 需要解释这些片段是如何连接并且得到答案的
还需要考虑什么样的训练素材，让我们可以达到这种程度的解释性。
...
当下的模型建造在 要么序列model 或者 各种单词对上面，缺少语言的内部结构。
一方面，这让我们的模型胡乱的学语言相关信息，
另一方面 nlp社区也努力学习 语言表达，building many linguistic resources and tools
language encode意味着 明确的分层，简介的架构。

如何把结构适配到序列模型里面，我们是想把这个结构作为隐藏变量还是现成的语言工具；现有工具好到能适配更多吗？如何提高他们的的表现

我们要在一个神经网络里面实现所有
如果我们想接触到更深层的阅读理解，那未来的模型要更structure，更model
...
4.4.2 representation vs architecture
有个趋势是增加神经网络的复杂度。特别是越来越复杂的注意简直被用于抓取段落和问题的相似性上面。

有个试验，是pretrain完大量text corpora后，
一个简单的问题段落串行模型，（其中问题和段落 没有任何直接联系的）在SQuAD上表现的也很好。

xxx xxx xxx xxx
这两种类型的模型有两种极端：
1.一个他证明力了无监督的不可思议的表现 （如果给强语言模型），强训练text后，一个简单的模型也可以足够。
2.如果给弱语言模型（只有word embedding），给出优先知识就很小心

以后我们需要结合两者，像bert这样处理问题就太粗暴了，需要更复杂的推理验算
4.4.3 how many training examples are needed？

1.训练数据多更有好处
2.pre-train可以减少对大数据量的依赖
（这些模型里，大多数参数已经pretrain过了，只需要微调）

日后，我们希望有更多研究在无监督学习和transfer学习上。
杠杆化未标签数据，或者其他cheap 来源，或者建监督 会把我们从标注数据上面解放出来。
我们也在找更好更便宜的监督数据库。
```
